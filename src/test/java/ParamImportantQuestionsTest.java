...
public class ParamImportantQuestionsTest extends BaseTest{
    private final int questionLocatorIndex;
    private final int answerLocatorIndex;
    private final String answerText;

    public ParamImportantQuestionsTest(int questionLocator, int answerLocator, String answerText){
        this.questionLocatorIndex = questionLocator;
        this.answerLocatorIndex = answerLocator;
...
//Проверяем текст
            String actualText = homePage
                    .clickQuestion(questionLocatorIndex)
                    .checkText(answerLocatorIndex);

            assertEquals("Текст не соответствует ожидаемому для вопроса " + questionLocatorIndex,
                    answerText,
                    actualText);
        }
}
